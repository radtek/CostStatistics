...
int CTreeGridCtrl::FindRowIndex(GTV_HTREENODE hNode)
{
	...
	{
		if(hNode == m_hRootNode)
		{
			return m_nRootRowIdx;
		}
		int nFixRowCount = GetFixedRowCount();
		if(nFixRowCount>0)
		{
			return nFixRowCount;
		}
...
GTV_HTREENODE CTreeGridCtrl::InsertNode(GTV_HTREENODE hParent, GTV_HTREENODE hInsertAfter, /*OUT*/int& nRowIndex)
{
	...
	{
		hParent = m_hRootNode;/*总根，第一级在总上添加*/
		
		if(!m_bHaveInsertNode)
		{
			m_nRootRowIdx = GetRowCount()-1;
			m_bHaveInsertNode = true;
		}
	...
	GTV_HTREENODE hNextItem = NULL; /*下一行*/
	GTV_HTREENODE hPrevItem = hInsertAfter; /*该行作为上一行 说明 都是向下下操作了*/

	if(hParent != NULL)
	{
		/*设定 上一行 下一行 的 hPrevItem hNextItem===================================================================*/
		if(!hParent->IsLeaf())/*有无子行*/
		{
			if(hInsertAfter == GLVI_FIRST)
			{
				...
				hNextItem = hParent->m_arChildNodes[0];/*第一个*/
			}
			else if(hInsertAfter == GLVI_LAST)
			{
				// Insert just after the row of the last child
				hPrevItem = hParent->m_arChildNodes[hParent->m_arChildNodes.GetSize() - 1];/*设为最后一个*/
				/*下一行 空*/
			}else /*插入之后 zhx增加 */
			{
				if(hPrevItem!=NULL) /*要添加 之后的 节点*/
					hNextItem = hPrevItem->m_hNextSibling;
			}//zhxend
		}
		else
		{
			// Insert just after the row of the parent
			hPrevItem = NULL; /*开始时，上一行空*/
		}

		hNewItem = CreateNode();/*添加新行==设新一行数据 上，下，nRow =======================================*/
		if(hNewItem != NULL)
		{
			hNewItem->m_hParent = hParent; /*设 父 */
			// hPrevItem = NULL implies the parent is a leaf node 
			// or hInsertAfter = GLVI_FIRST when parent is not a leaf node
			if(hPrevItem == NULL) /*添加在最上面-1行 表头*/
			{
				// If previous item is NULL, then the newly created item
				// can only have a next sibling, but not a prev sibling
				hNewItem->m_hPrevSibling = NULL;   /*上一个*/
				hNewItem->m_hNextSibling = hNextItem; /*下一个*/

				if(NULL != hNextItem)
					hNextItem->m_hPrevSibling = hNewItem; /*下一个的上指向新 zhx增*/

				// Insert just after parent
				//nRowIndex = FindRowIndex(hParent) + 1;

				if(GetRowCount()== GetFixedRowCount() && GetRowCount() >0)
				{
					nRowIndex = GetRowCount();
				}
				else
				{
					if (hParent->IsLeaf())
			...
			{
				hNewItem->m_hPrevSibling = hPrevItem; /*上一个*/

				hNewItem->m_hNextSibling = hNextItem; /*下一个zhx增*/
				...
				hPrevItem->m_hNextSibling = hNewItem; /*上一个的下指向新*/
				if(NULL != hNextItem) //zhxadd
					hNextItem->m_hPrevSibling = hNewItem; /*下一个的上指向新 zhx增 zhxend*/
			...
			if(nNodeIndex > -1)
			{
				...
				hParent->m_arChildNodes.InsertAt(nNodeIndex, hNewItem);/*添加到 总根 的子节点数组 */
			}
			else /*序号错了，失败*/
			{
				DeleteNode(hNewItem);
...
BOOL CTreeGridCtrl::ShowRow(GTV_HTREENODE hItem, int nRow, BOOL bShow)
{
	...
	if(!bShow)
	{
		int nHet = GetRowHeight(nRow);
		if(nHet>0)
		{
			m_mapHiddenRowHeights.SetAt(hItem,nHet );
		}
		else
		{
...
BOOL CTreeGridCtrl::SetCellTypeID(int nRow, int nColumn, enum CellTypeID cellType)
{
	...
	switch (cellType)
	{
		case CT_DEFAULT:
		{
			bRet = CGridCtrl::SetCellType(nRow, nColumn, RUNTIME_CLASS(CTreeGridCell)) != NULL;
		}
		break;
		
		case CT_CHECKBOX:
		{
			bRet = CGridCtrl::SetCellType(nRow, nColumn, RUNTIME_CLASS(CTreeGridCellCheck)) != NULL;
		}
		break;		
	}
...
int CTreeGridCtrl::GetNodeLevel(GTV_HTREENODE hNode)
{
	int level = 0;
	GTV_HTREENODE hTmp = hNode;
	while(hTmp != NULL)
	{
		hTmp = hTmp->m_hParent;
		level++;
