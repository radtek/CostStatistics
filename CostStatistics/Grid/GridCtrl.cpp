...
{
	m_nAutosizeFromRow = 0;
	m_bAutoWordBreak = true;
    RegisterWindowClass();
m_bCreate = false;
	...
	m_bShowScrollBar_Hoz = TRUE;
	m_bShowScrollBar_Vert = TRUE;
    ...
    SetPrintMarginInfo(4, 2, 4, 4, 1, 1, 1);
#endif	
...
CGridCtrl::~CGridCtrl()
{
	...
	if (m_bNeedDelImageList && m_pImageList)
	{
		delete m_pImageList;
		m_pImageList = NULL;
	}
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	...
	if(m_hScrollBar.m_hWnd==NULL)
	{
		CRect rcScroll(0,0,0,0);
		m_hScrollBar.Create(WS_CHILD|SBS_HORZ,rcScroll, this, AFX_IDW_HSCROLL_FIRST);
		m_vScrollBar.Create(WS_CHILD|SBS_VERT,rcScroll, this, AFX_IDW_VSCROLL_FIRST);
	}
...
void CGridCtrl::PreSubclassWindow()
{
    ...
    //DeleteObject(hFont);
if(!m_bCreate && m_hScrollBar.m_hWnd==NULL)
	{
		CRect rcScroll(0,0,0,0);
		m_hScrollBar.Create(WS_CHILD|SBS_HORZ,rcScroll, this, AFX_IDW_HSCROLL_FIRST);
		m_vScrollBar.Create(WS_CHILD|SBS_VERT,rcScroll, this, AFX_IDW_VSCROLL_FIRST);
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage,int nOther) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
void CGridCtrl::OnSize(UINT m_nType, int cx, int cy)
{  
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
    ...
    case SB_LINEDOWN:
        if (scrollPos < m_nVScrollMax)
        {
			...
			///////////////////////////////往父窗口发送滚动条滚到底的消息 wushunli 2016/08/12 begin/////////////////////////////////////////////
			if (GetScrollPos32(SB_VERT) == scrollPos)
			{
				m_dwEndTime		= ::GetTickCount();
				if( m_dwEndTime - m_dwStartTime < 100 )
				{
					break;
				}
				
				::PostMessage(this->GetParent()->m_hWnd,WM_MSG_HSCROLL_END,NULL,NULL);
				m_dwEndTime = ::GetTickCount();
				m_dwStartTime = m_dwEndTime;

				break;          // didn't work
			}
           //////////////////////////////////往父窗口发送滚动条滚到底的消息 wushunli 2016/08/12 end//////////////////////////////////////////
    ...
    case SB_PAGEUP:
        if (scrollPos > 0)
        {
			...
			////////////////////////////////wushunli add begin//////////////////////////////////////////
			::PostMessage(this->GetParent()->m_hWnd,WM_MSG_HSCROLL_UP,NULL,NULL);
			////////////////////////////////wushunli add end//////////////////////////////////////////
        }
        break;
	...
	::PostMessage(this->GetParent()->m_hWnd,WM_MSG_VSCROLL,NULL,NULL);
}
...
void CGridCtrl::SetScrollInfo(int nBar, LPSCROLLINFO lpScrollInfo, BOOL bRedraw /* = TRUE */)
{
	if(nBar == SB_VERT)
	{
		m_vScrollBar.SetScrollInfo(lpScrollInfo,bRedraw);
	}
	if(nBar == SB_HORZ)
	{
		m_hScrollBar.SetScrollInfo(lpScrollInfo,bRedraw);
	}
}
...
CCellID CGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    int nVertScroll = m_vScrollBar.GetScrollPos(), 
        nHorzScroll =m_hScrollBar. GetScrollPos();
...
int CGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
	...
	CScrollBar* pScroll =(nBar == SB_VERT ? &m_vScrollBar : &m_hScrollBar);

    if (bGetTrackPos)
    {
        if (pScroll-> GetScrollInfo( &si, SIF_TRACKPOS))
            return si.nTrackPos;
    }
    else
    {
        if (pScroll->GetScrollInfo( &si, SIF_POS))
            return si.nPos;
    }
...
BOOL CGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	...
	if(nBar == SB_VERT)
	{
		return m_vScrollBar.SetScrollInfo(&si,bRedraw);
	}
	else if(nBar == SB_HORZ)
	{
		return m_hScrollBar.SetScrollInfo(&si,bRedraw);
	}
...
{
	if(m_hScrollBar.m_hWnd == NULL) return;

	CRect rc;
	GetClientRect(&rc);
	int widS = GetSystemMetrics(SM_CXVSCROLL);
	int hetS = GetSystemMetrics(SM_CYHSCROLL);


	
    if (bEnable)
    {
        if (!IsVisibleHScroll() && (nBar == SB_HORZ || nBar == SB_BOTH) && m_bShowScrollBar_Hoz)
        {
        ...
        if (!IsVisibleVScroll() && (nBar == SB_VERT || nBar == SB_BOTH) && m_bShowScrollBar_Vert)
        {
		...
		if(IsVisibleVScroll())
		{
			...
			m_vScrollBar.MoveWindow(rc.right-widS,0,widS,rc.Height());
		}

		if(IsVisibleHScroll())
		{
			CRect rcH(0,rc.bottom-hetS,rc.right,rc.bottom);
			if(IsVisibleVScroll())
			{
				rcH.right -= widS;
			}
			m_hScrollBar.MoveWindow(&rcH);
		}
...
LRESULT CGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
	...
	CFont tmpF;
	tmpF.CreateFontIndirect(&lf);

	CFont *pOldFt = dc.SelectObject(&tmpF);

	int het = dc.GetTextExtent("哈8").cy;
	int margin = GetDefCellMargin();

	het += GetDefCellMargin() * 2;
	dc.SelectObject(pOldFt);
	SetDefCellHeight(het);
    Refresh();
...
// Cell creation stuff
 ...
 CGridCellBase*  CGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
    if (GetVirtualMode())
        return NULL;
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
	...
	if(m_nFixedRows+m_nFreezedRows>m_nRows && m_nFreezedRows>0)
	{
		m_nFreezedRows--;
	}
	...
	// 重新计算合并单元格范围，防止重绘时崩溃
	for (int i = 0; i < m_arMergedCells.GetSize(); ++i)
	{
		if (m_arMergedCells[i].GetMinRow() == nRow &&
			m_arMergedCells[i].GetMaxRow() == nRow)
		{
			m_arMergedCells.RemoveAt(i--);
		}
		else
		{
			if (m_arMergedCells[i].GetMinRow() == nRow)
			{
				m_arMergedCells[i].SetMinRow(nRow);
			}
			else if (m_arMergedCells[i].GetMinRow() > nRow)
			{
				int nMinRow = m_arMergedCells[i].GetMinRow() - 1;
				m_arMergedCells[i].SetMinRow(nMinRow);
			}

			if (m_arMergedCells[i].GetMinRow() >= nRow)
			{
				int nMaxRow = m_arMergedCells[i].GetMaxRow() - 1;
				m_arMergedCells[i].SetMaxRow(nMaxRow);
			}
		}
	}
...
BOOL CGridCtrl::DeleteAllItems()
{
    ...
    m_idCurrentCell.row = m_idCurrentCell.col = -1;
    m_nRows = m_nFixedRows = m_nCols = m_nFixedCols =m_nFreezedCols = m_nFreezedRows= 0;
...
static bool IsLegalDate(int year, int month, int day)
{
...
static int StringToVector(const std::string& srcStr, std::vector<std::string>& destVect, const std::string& strFlag)  
{  
	destVect.clear();
...
void StringReplaceAll( std::string &strIn, const std::string &strSrc, const std::string &strDest )
{
	...
	catch (...)
	{
		...
		//PRINT_LOG("StringReplaceAll exception!");
	}
}

void GetRidofNoneNumber(std::string strData,std::vector<std::string>& vec)
{	
	vec.clear();

	std::string strDate = strData;
	std::string strTmp = "";

	for (int i = 0; i <= strDate.size(); i++)
	{
		int tmp = (int)strDate[i];
		if (tmp >= 48 && tmp <= 57)
		{
			strTmp += strDate[i];
			continue;
...
static int CompareDate(CString strDate1,CString strDate2)
{
	std::string str1 = strDate1;
	std::string str2 = strDate2;
	...
	//去掉非数字的字符
	GetRidofNoneNumber(str1,vec1);
	GetRidofNoneNumber(str2,vec2);

	if( vec1.size() >= 3 )
	{
		nYear1 = atoi(vec1.at(0).c_str());
		nMonth1 = atoi(vec1.at(1).c_str());
		nDay1 = atoi(vec1.at(2).c_str());

		if( vec1.size() == 6 )
		{
			nHour1 = atoi(vec1.at(3).c_str());
			nMinute1= atoi(vec1.at(4).c_str());
			nSecond1 = atoi(vec1.at(5).c_str());
		}
	}

	if( vec2.size() >= 3 )
	{
		nYear2 = atoi(vec2.at(0).c_str());
		nMonth2 = atoi(vec2.at(1).c_str());
		nDay2 = atoi(vec2.at(2).c_str());

		if( vec2.size() == 6 )
		{
			nHour2 = atoi(vec2.at(3).c_str());
			nMinute2= atoi(vec2.at(4).c_str());
			nSecond2 = atoi(vec2.at(5).c_str());
		}
		
	}

	int tm1 = nYear1 * 10000 + nMonth1 * 100 + nDay1;
	int tm2 = nYear2 * 10000 + nMonth2 * 100 + nDay2;
	if( tm1 != tm2 )
	{
		return ( tm1 > tm2 ) ? 1:-1;
	}

	tm1 = nHour1 * 3600 + nMinute1 * 60 + nSecond1;
	tm2 = nHour2 * 3600 + nMinute2 * 60 + nSecond2;
	if( tm1 != tm2 )
	{
		return ( tm1 > tm2 ) ? 1:-1;
	}
...
BOOL CGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	...
	bool bIsDate = false;
	StringReplaceAll(strText,":","-");
	StringReplaceAll(strText," ","-");

	strText.erase(std::remove(strText.begin(), strText.end(), '.'), strText.end());
	if( strText.find("-") != std::string::npos || strText.find("/") != std::string::npos ) //日期
	{
		std::vector<std::string> vec;
		StringToVector(strText,vec,"-");
		if( vec.size() >= 3 )
		{
			bIsDate = IsLegalDate(atoi(vec.at(0).c_str()),atoi(vec.at(1).c_str()),atoi(vec.at(2).c_str()));
		}
		//strText.erase(std::remove(strText.begin(), strText.end(), '-'), strText.end());
	}
	...
	if( bIsDate )
	{
		m_pfnCompare = pfnCellDateCompare;
	}
	else if( AllisNum(strText) )//数字
	{
                          ...
                          LPARAM data /* = 0 */)
{
    ...
    ResetSelectedRange();
    SetFocusCell(-1, -1);
    return SortItems(pfnCompare, nCol, bAscending, data, GetFixedRowCount(TRUE), -1,GetIncludeLastRow());
}
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	CString txt1 = pCell1->GetText();
	CString txt2 = pCell2->GetText();
	
	
	string pinyin1, pinyin2;
	CLetterConvert::GetPinYin(txt1,pinyin1);
	CLetterConvert::GetPinYin(txt2,pinyin2);
	
	//return _tcscmp(pCell1->GetText(), pCell2->GetText());
	int ret= _tcscmp(pinyin1.c_str(),pinyin2.c_str());
...
int CALLBACK CGridCtrl::pfnCellDateCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
	...
	CString strDate1 = pCell1->GetText();
	CString strDate2 = pCell2->GetText();
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
                          int low, int high,BOOL bIncludeLastRow)
{
                               ...
                               BOOL bResetScroll /*=TRUE*/)
{
	...
	if(m_nAutosizeFromRow !=0)
	{
		nStartRow = m_nAutosizeFromRow;
	}
    if (GetVirtualMode())
...
BOOL CGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
    ...
    for (int nCol = 0; nCol < nNumColumns; nCol++)
    {
        ...
        if (pCell)
		{
			...
			CRect rcCell;
			GetTextRect(nRow,nCol,&rcCell);
			rcCell.bottom = rcCell.top+10;
			if(rcCell.right-rcCell.left>5)
			{
				CFont *pOldFont = pDC->GetCurrentFont();
				pDC->SelectObject(pCell->GetFontObject());
				CRect rcx = MyDrawText(pDC,0,0,pCell->GetText(),rcCell,true);
				size = rcx.Size();
				size.cy += pCell->GetMargin()*2;
...
void CGridCtrl::AutoSizeColumns(UINT nAutoSizeStyle,int maxWid /*=GVS_DEFAULT*/)
{
    int nNumColumns = GetColumnCount();
    for (int nCol = 0; nCol < nNumColumns; nCol++)
    {
		...
		int wid = GetColumnWidth(nCol);
		if(wid>maxWid)
		{
			SetColumnWidth(nCol,maxWid);
		}
    }
    ResetScrollBars();
}
...
void CGridCtrl::EnableAutoWordbreak(bool bEnable)
{
	m_bAutoWordBreak = bEnable;
}
...
BOOL CGridCtrl::InvalidateRow(int row)
{
	int nCol = GetColumnCount();
	for (int c = 0; c < nCol; ++c)
	{
		InvalidateCellRect(row, c);
	}
	return TRUE;
} 
/////////////////////////////////////////////////////////////////////////////
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		CRect cellRect;
		GetCellRect(m_LeftClickDownCell.row, m_LeftClickDownCell.col, cellRect);
	...
	BOOL bEditable = IsCellEditable(m_LeftClickDownCell);
    if (m_LeftClickDownCell == m_idCurrentCell && 
        !(nFlags & MK_CONTROL) && bInTextArea && bEditable
        )
    {
...
void CGridCtrl::Print(CPrintDialog* pPrntDialog /*=NULL*/)
{
    ...
    CString strTitle = m_strPrintHeader;
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
 ...
 /*   CString strRight;
    ...
    CString strCenter = m_strPrintHeader;
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
    ...
    if (GetHeaderSort() && (IsTextSortColumn(cell.col) || IsNumericSortColumn(cell.col)) )
    {
        CWaitCursor waiter;
        if (cell.col == GetSortColumn())
		{
			if (IsTextSortColumn(cell.col))
			{
				m_pfnCompare = NULL;
			}

			if (IsNumericSortColumn(cell.col))
			{
				m_pfnCompare = pfnCellNumericCompare;
			}
...
void CGridCtrl::GetMergeCells(CArray<CCellRange, CCellRange&>& arr)
{
	arr.Copy(m_arMergedCells);
}
...
{ 	
	...
	if(idTopLeftMin.row>=GetRowCount()) return FALSE;
...
void CGridCtrl::ShowScrollBar(BOOL bHoz, BOOL bVert)
{
	m_bShowScrollBar_Hoz = bHoz;
	m_bShowScrollBar_Vert = bVert;
	if(m_hScrollBar.m_hWnd)
	{
		m_hScrollBar.ShowWindow(m_bShowScrollBar_Hoz ? SW_SHOW:SW_HIDE);
		m_vScrollBar.ShowWindow(m_bShowScrollBar_Vert ? SW_SHOW:SW_HIDE);
	}
}
// LUC
...
void CGridCtrl::GetSelectedRows(set<int> & rows) const
{
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		DWORD key;
...
void CGridCtrl::SetColState(int nCol, int nState, int nRowFrom)
{
	int nRows = GetRowCount();

	for (int r = nRowFrom; r < nRows; ++r)
	{
		SetItemState(r, nCol, nState);
	}
}

void CGridCtrl::SetRowBkColour(int nRow, COLORREF clr)
{
	for(int i=GetFixedColumnCount();i<GetColumnCount();i++)
	{
		SetItemBkColour(nRow,i,clr);
	}
}

bool CGridCtrl::IsTextSortColumn(int col)
{
	  vector<int>::iterator iter=find(m_vSortTextColVec.begin(),m_vSortTextColVec.end(),col);
	  if( iter !=m_vSortTextColVec.end())
		  return true;
	  else
		  return false;
...
bool CGridCtrl::IsNumericSortColumn(int col)
{
	vector<int>::iterator iter=find(m_vSortNumericColVec.begin(),m_vSortNumericColVec.end(),col);
	if( iter !=m_vSortNumericColVec.end())
		return true;
	else
		return false;
...
void CGridCtrl::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	m_nHitTest = nHitTest;
...
void CGridCtrl::PrintGrid(CString title)
{
	m_strPrintHeader = title;
	Print();
}
void CGridCtrl::OnGridPrint(CDC* pDC, CPrintInfo* pInfo)
{
	OnPrint(pDC,pInfo);
}
void CGridCtrl::OnGridBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
	OnBeginPrinting(pDC,pInfo);
}
void CGridCtrl::OnGridEndPrinting(CDC* pDC, CPrintInfo* pInfo)
{
