...
void CComboEdit::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if(pOwner)
	{
		int index=pOwner->FindInputString(str);
		if(index!=CB_ERR)
			pOwner->SetCurSel(index);
	}
...
int  CInPlaceList::FindInputString(CString & str)
{
	if(str.Trim().IsEmpty())
		return CB_ERR;
	int count = GetCount();
...
BOOL CGridCellCombo::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (GetGrid()->IsCellEditable(nRow, nCol) && !IsEditing())
    {
        // Get the size of the scroll box
			...
			ScrollRect.left = rect.right-16;
			
			CPoint cc = ScrollRect.CenterPoint();

			POINT pts[]={{cc.x-3,cc.y-2},{cc.x+3,cc.y-2},{cc.x,cc.y+1}};
			CBrush br(RGB(0,0,0));
...
void CGridCellCombo::SetData(const CStringArray& ar)
{
	m_arrData.RemoveAll();
	for (int i = 0; i < ar.GetSize(); i++)
		m_arrData.Add(ar[i]);
...
void CGridCellCombo::InsertString(int nPos,CString str)
{
	m_Strings.InsertAt(nPos,str);
}
