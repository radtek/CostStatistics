...
typedef struct tagNM_GRIDVIEW {
    ...
    int   iRow;
    int   iColumn;
	int iIdx;				// for multi button index
} NM_GRIDVIEW;
...
#define WM_MSG_VSCROLL		WM_USER + 10001
#define WM_MSG_HSCROLL		WM_USER + 10002
#define WM_MSG_HSCROLL_END	WM_USER + 10003
#define WM_MSG_HSCROLL_UP	WM_USER + 10004
...
#define GVN_ENDFILLDATA			(LVN_ODFINDITEM - 1)
#define GVN_CHANGECOLWID		(GVN_ENDFILLDATA - 1)
#define GVN_BTNCLICK			(GVN_CHANGECOLWID - 1)
#define GVN_ENDCELLEDIT			(GVN_BTNCLICK - 1)
#define GVN_CHKCLICK			(GVN_ENDCELLEDIT - 1)
#define GVN_CMBCHANGE			(GVN_CHKCLICK - 1)
#define GVN_ENDTREEDRAG			(GVN_CMBCHANGE - 1)
#define GVN_VKUPDOWN			(GVN_ENDTREEDRAG - 1)
#define  GVN_URLCLICK   (GVN_VKUPDOWN-1)
class CGridCtrl;
...
class CGridCtrl : public CWnd,public IGridCtrlPrintSupport
{
...
///// LUC ///////////////////////////////////////////////////////////////////////
	...
	void SplitCells(INT_PTR nMergeID);
	 void SetColState(int nCol, int nState, int nRowFrom = 2);
	...
	void ShowScrollBar(BOOL bHoz, BOOL bVert);
	BOOL	m_bDrawingMergedCell;
	INT_PTR	m_nCurrentMergeID;
	int m_nAutosizeFromRow;
...
public:
	...
	void SetRowBkColour(int nRow, COLORREF clr);
	...
	void EnableAutoWordbreak(bool bEnable);
    ...
    void SetImageList(CImageList* pList, bool bNeedDel = false)          { m_pImageList = pList; m_bNeedDelImageList = bNeedDel; }
	...
	void SetTextSortColumns(const vector<int> & vec)  {m_vSortTextColVec =  vec;        }
	void SetTNumericSortColumns(const vector<int> & vec)  {m_vSortNumericColVec =  vec;        }
	bool IsTextSortColumn(int col);
	bool IsNumericSortColumn(int col);
...
public:
    ...
    CGridCellBase* SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass);
...
public:
    ...
    void AutoSizeRows();
    void AutoSizeColumns(UINT nAutoSizeStyle = GVS_DEFAULT,int maxWid = 1000);
...
///////////////////////////////////////////////////////////////////////////////////
 public:
	...
	void SetScrollInfo(int nBar, LPSCROLLINFO lpScrollInfo, BOOL bRedraw = TRUE );
///////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	void SetIcludeLastRow(BOOL bIncludeLastRow = FALSE){     m_bIncludeLastRow = bIncludeLastRow;    }
	void SetCompareFunction(PFNLVCOMPARE pfnCompare);
	BOOL GetIncludeLastRow(){   return  m_bIncludeLastRow;   }
	...
	static int CALLBACK pfnCellDateCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
...
public:
	...
	BOOL InvalidateRow(const int row);//zhx
...
protected:
    ...
    LRESULT SendMessageToParent(int nRow, int nCol, int nMessage, int nOther = 0) const;
    ...
    BOOL SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
                   int low, int high,BOOL bIncludeLastRow = TRUE);
...
public:
	...
	void SetDefCellFont(const LOGFONT* lgft)
	{
		m_cellDefault.SetFont(lgft); 
		m_cellFixedRowDef.SetFont(lgft); 
		m_cellFixedColDef.SetFont(lgft); 
		m_cellFixedRowColDef.SetFont(lgft); 
	}

	void GetMergeCells(CArray<CCellRange, CCellRange&>& arr);
protected:
	UINT m_nHitTest;
	bool m_bNeedDelImageList;
...
protected:
    ...
    int          m_bAscending;
    int          m_nSortColumn;
	vector<int>  m_vSortTextColVec; //文本排序列
	vector<int>  m_vSortNumericColVec; //数值排序列
 	BOOL         m_bIncludeLastRow;
...
protected:
	...
	BOOL m_bShowScrollBar_Hoz;
	BOOL m_bShowScrollBar_Vert;
    ...
    afx_msg void OnSize(UINT m_nType, int cx, int cy);
...
//add by wupuliang
	...
	virtual void PrintGrid(CString title);
