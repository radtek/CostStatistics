...
void DotLine(CDC *pDC, CPoint from, CPoint to)
{
	if(from.y>to.y)
	{
		int tmp = from.y;
	...
	if(from.x == to.x)
	{
		for(int i=from.y;i<=to.y;  i+=2)
		{
			pDC->SetPixelV(from.x,i,clr);
		}
	}

	if(from.y == to.y)
	{
		for(int i=from.x;i<=to.x;  i+=2)
		{
			pDC->SetPixelV(i,from.y,clr);
		}
	}
}
...
{	
	...
	else
	{
		...
		{
			pen.CreatePen(PS_SOLID,0,RGB(120,120,120));
		}
		else
		{
				...
				&& (pGridCtrl->GetVertTreeLineColor() != CLR_NONE))
		{
			...
			if(nHorzLineStartX < rect.right)
			{
				...
				DotLine(pDC,CPoint(nHorzLineStartX, ptOrigin.y),CPoint(nHorzLineStartX, nParentTreeBtnImgY));
			}
