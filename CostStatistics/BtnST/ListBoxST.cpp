...
CListBoxST::CListBoxST()
{
	...
	m_ItemBkColor=GetSysColor(COLOR_WINDOW);
	m_SelectedItemBkColor = GetSysColor(COLOR_HIGHLIGHT);
}
...
BEGIN_MESSAGE_MAP(CListBoxST, CListBox)
	...
	//}}AFX_MSG_MAP
	ON_CONTROL_REFLECT(LBN_SETFOCUS, &CListBoxST::OnLbnSetfocus)
	ON_WM_MOUSEMOVE()
END_MESSAGE_MAP()
...
void CListBoxST::DrawItem(LPDRAWITEMSTRUCT pDIStruct)
{
	...
	COLORREF		crNormal = m_ItemBkColor;
	
	COLORREF		crSelected =m_SelectedItemBkColor; 
	...
	bool bhover = pDIStruct->itemState &  ODS_HOTLIGHT;
...
void CListBoxST::SetItemBkBColor(COLORREF color)
{
	m_ItemBkColor=color;
}

void CListBoxST::SetSelectedItemBkBColor(COLORREF color)
{
	m_SelectedItemBkColor=color;
}
...
void CListBoxST::OnLbnSetfocus()
{
	int i=0;// TODO: 在此添加控件通知处理程序代码
}

void CListBoxST::OnMouseMove(UINT nFlags, CPoint point)
{
