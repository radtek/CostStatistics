...
#define TLC_IMAGE_PLUS				0x00800000				// show image plus
...
#define TLHT_ONSHADOWBUTTON			0x0040					// on shadow button
...
#define TLN_ITEM_CLICK              (TVN_FIRST-32)
#define TLN_ITEM_DCLICK             (TVN_FIRST-33)
#define TLN_ITEM_RCLICK             (TVN_FIRST-36)

#define  TLN_HEAD_DCLICK         (TVN_FIRST-37)
#define  TLN_HEAD_FILTER          (TVN_FIRST-38)

#define  MSG_TREELIST_SCROLL   8971
...
typedef void (*PFuncTreeListCtrlDblClk)(CWnd *pOwnerWnd, CTreeListCtrl *pTeeListCtrl, CTreeListItem *pItem);

typedef void (*PFuncTreeListCtrlCreateContextMenu)(CWnd *pOwnerWnd, CTreeListCtrl* pTeeListCtrl, CTreeListItem* pItem, CMenu* pMenu);

typedef void (*PFuncTreeListCtrlContextMenuClick)(CWnd *pOwnerWnd, CTreeListCtrl* pTeeListCtrl, CTreeListItem* pItem, UINT nMenuId);

typedef void (*PFuncTreeListCtrlItemChecked)(CWnd *pOwnerWnd, CTreeListCtrl* pTeeListCtrl, CTreeListItem* pItem);
...
class CTreeListDropFileHandler
{
public:
	virtual void OnFileDrop(CTreeListCtrl* pCtrl, CTreeListItem* pItem , vector<CString> &files) = 0;
};
...
public:
	...
	bool GetEnableFilter(){return m_bFilter;}
protected:
	 BOOL Create();
	void DrawFixedCoumn(CDC* pDC,CRect rcClip);
// Members
protected:
...
private:
	...
	COleDropTargetEh m_OleDropTargetEh;
    IDropTargetHelper* m_piDropHelper;
    bool m_bUseDnDHelper;

	bool m_bShiftMutiSelecet;
	CTreeListItem* m_pShiftMutiSelecteBeginItem;

	UINT m_nHitTest;
	BOOL m_bHideHScroll;

    BOOL     m_bCloseOleDropTarget;
// Attributes
public:
	// ***************************** CTreeCtrl ******************************
	int		GetCount();
	void DeleteShadowItem(CShadowItem* pShadow);
	...
	int GetOffspringNodes(CArray<CTreeListItem*>& arr,CTreeListItem* pItem);
	...
	BOOL	GetHideHScroll() const			{return m_bHideHScroll;	}
	CTreeListItem* GetRootItem();
	CTreeListItem* GetRealRootItem();
	void	EnableHideHScroll(BOOL bHide)	{m_bHideHScroll = bHide;	}
	...
	void	GetItemImagePlus( CTreeListItem* pItem, int& nImage, int& nExpandImage, int& nSelectedImage, int& nExpandSelectedImage );
	void	SetItemImagePlus( CTreeListItem* pItem, int nImage, int nExpandImage, int nSelectedImage, int nExpandSelectedImage );
	...
	DWORD	GetItemData( CTreeListItem* pItem,int nSubItem=0 );
	void	SetItemData( CTreeListItem* pItem, DWORD dwData,int nSubItem=0 );
	DWORD	GetItemData2( CTreeListItem* pItem );
	void	SetItemData2( CTreeListItem* pItem, DWORD dwData );
	...
	void SetItemBkColor(CTreeListItem * pItem, COLORREF clrBk);
	void SetItemTextColor(CTreeListItem * pItem, COLORREF clr);
	...
	DWORD	GetItemData2( int nItem );
	void	SetItemData2( int nItem, DWORD dwData );
	...
	bool m_bAllowSelectNull;
	// *************************** CTreeListCtrl ****************************
	void	SetFont(CFont* pFont = NULL);
	CFont*	GetFont();
	...
	void	MoveUp( CTreeListItem* pItem );
	void	MoveDown( CTreeListItem* pItem );
	void    MoveTop( CTreeListItem* pItem );
	...
	CTreeListItem* NextItem( CTreeListItem* pItem, bool bChild = true);
	void ExpandAll();

    void CloseOleDropTarget(BOOL bClose=TRUE);

	void SetPadTreeColumn(int nPadCol);
...
public:
	...
	CShadowItem* AddShadowItem( LPCTSTR lpszItem, CTreeListItem* pParent, int nImage=-1 );
	
	CShadowItem* FindShadowItem(CTreeListItem *pItem);
	

	int GetShadowCount(CTreeListItem* pItem);
	...
	void	SelectItem( CTreeListItem* pItem, int iSubItem = 0, BOOL bClearLastSelected = TRUE,BOOL bMoveScrollH = TRUE );
	void	ActiveItem( CTreeListItem* pItem, int iSubItem = 0 );
	void	ShiftMuitiSelect(CTreeListItem* pItem1, CTreeListItem* pItem2, int iSubItem);


	void	SetTargetItem( CTreeListItem* pItem = NULL );
	void	SetHoverItem( CTreeListItem* pItem = NULL );

	void SetFixedCoumnCount(int nCount);
...
public:
	...
	CTreeListDropFileHandler *m_pFileDropHandler;
...
protected:
	...
	void	DrawItem( CDC* pDC, CRect rcItem, CTreeListItem* pItem ,BOOL bFixed=FALSE);
	...
	CRect	CalcLineRect(CRect rcColumn, CTreeListItem* pItem);
...
public:
	...
	void InsertDropItems(vector<CString> &files);

	virtual DROPEFFECT OnDropEx(COleDataObject* pDataObject,
		DROPEFFECT dropDefault, DROPEFFECT dropList, CPoint point);
	virtual DROPEFFECT OnDragScroll(DWORD dwKeyState, CPoint point);

    BOOL ReadHdropData(COleDataObject* pDataObject, vector<CString> &files);
    void HighlightOverItem(CPoint point);

	void CollapseItemShadow(CTreeListItem* pItem);
	void ExpandItemShadow(CTreeListItem* pItem);
...
public:
	...
	vector<CString> *m_pDragItems;
	PFuncTreeListCtrlDblClk m_pFuncTreeListCtrlDblClk;
	PFuncTreeListCtrlCreateContextMenu m_pFuncTreeListCtrlCreateContextMenu;
	PFuncTreeListCtrlContextMenuClick m_pFuncTreeListCtrlContextMenuClick;
	PFuncTreeListCtrlItemChecked m_pFuncTreeListCtrlItemChecked;
	CWnd *m_pOwner;
...
protected:
	CMenu m_menu;
	CTreeListItem* m_pItemContextMenuOn;
	void CreateContextMenu(CPoint point);
	virtual BOOL OnCommand(WPARAM wParam, LPARAM lParam);

	int m_nFixedCoumnCount;
