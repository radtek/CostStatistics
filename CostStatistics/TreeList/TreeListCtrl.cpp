...
CTreeListCtrl::CTreeListCtrl() : 
    ...
    m_minSize(1,1),
    m_bCareDblClk(false),
	m_bShiftMutiSelecet(false),
	m_pShiftMutiSelecteBeginItem(NULL),
	m_nHitTest(0),
	m_pDragItems(NULL),
	m_pItemContextMenuOn(NULL),
	m_bHideHScroll(FALSE),
	m_bContextMenu(false),
	m_bCareCheck(false),
	m_pFuncTreeListCtrlDblClk(NULL),
	m_pFuncTreeListCtrlCreateContextMenu(NULL),
	m_pFuncTreeListCtrlContextMenuClick(NULL),
	m_bMouseDown(FALSE),
	m_pFileDropHandler(NULL),
	m_nPadTreeCol(0)
{
	m_bFilter = false;
	m_nFixedCoumnCount = 0;
	m_bLayout = false;
	// construct
	m_bAllowSelectNull = true;
...
BOOL CTreeListCtrl::Create()
{
	...
	ASSERT(m_imgTree.m_hImageList != NULL);
    ...
    if( !m_bCloseOleDropTarget )
    {
	    BOOL ret = m_OleDropTargetEh.Register(this);
...
void CTreeListCtrl::SetHorzScroll( CRect* pPreClient )
{
	...
	if( pPreClient->Width() < GetWidth() && !GetHideHScroll())
	{
...
void CTreeListCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	GetParent()->PostMessage(MSG_TREELIST_SCROLL,(WPARAM)this,si.nPos);
	Invalidate();
}
...
void CTreeListCtrl::SetImageList( CImageList* pImageList )
{
	...
	ASSERT(pImageList->m_hImageList != NULL);
	m_pImageList = pImageList;

	Invalidate();
}

void CTreeListCtrl::DeleteShadowItem(CShadowItem* pShadow)
{
	if(pShadow)
	{
			for(int i=0;i<pShadow->m_pParent->m_shadowList.size();i++)
			{
				if(pShadow->m_pParent->m_shadowList[i] == pShadow)
				{
					pShadow->m_pParent->m_shadowList.erase(pShadow->m_pParent->m_shadowList.begin()+i);
					break;
				}
			}
			
			if(pShadow->m_pRealNode)
			{
				DeleteItem(pShadow->m_pRealNode);
...
int CTreeListCtrl::GetOffspringNodes(CArray<CTreeListItem*>& arr,CTreeListItem* pItem)
{
	if(pItem == NULL) return 0;
	if(pItem->m_pChild == NULL) return 0;

	CTreeListItem* pCur = pItem->m_pChild;
	while(pCur)
	{
		arr.Add(pCur);
		GetOffspringNodes(arr,pCur);
		pCur = pCur->m_pNext;
	}
...
CTreeListItem* CTreeListCtrl::GetRealRootItem()
{
	// retrieve root item of treelist control
	return m_pRootItem;
}
...
void CTreeListCtrl::GetItemImagePlus( CTreeListItem* pItem, int& nImage, int& nExpandImage, int& nSelectedImage, int& nExpandSelectedImage )
{
	// retrieve images of item specified by pItem
	ASSERT( pItem != NULL );
	pItem->GetImagePlus( nImage, nExpandImage, nSelectedImage, nExpandSelectedImage );
}
...
void CTreeListCtrl::SetItemImagePlus( CTreeListItem* pItem, int nImage, int nExpandImage, int nSelectedImage, int nExpandSelectedImage )
{
	// set images of item specified by pItem
	ASSERT( pItem != NULL );
	pItem->SetState( TLIS_SHOWTREEIMAGE );
	pItem->SetImagePlus( nImage, nExpandImage, nSelectedImage, nExpandSelectedImage );
	Invalidate();
}	
...
BOOL CTreeListCtrl::SetItemText( CTreeListItem* pItem, LPCTSTR lpszText, int nSubItem )
{
	...
	if(bRet)
	{
		if(pItem->m_bShadow)
		{
			CShadowItem *pShadow = FindShadowItem(pItem);
			pShadow->SetText(lpszText,nSubItem);
		}
	}
	Invalidate();
	return bRet;
...
DWORD CTreeListCtrl::GetItemData( CTreeListItem* pItem,int nSubItem )
{
	// retrieve 32-bit data of item specified by pItem
	ASSERT( pItem != NULL );
	return pItem->GetData(nSubItem);
}

void CTreeListCtrl::SetItemData( CTreeListItem* pItem, DWORD dwData,int nSubItem )
{
	// set 32-bit data of item specified by pItem
	ASSERT( pItem != NULL );
	pItem->SetData( dwData,nSubItem );

	CShadowItem *pShadow = FindShadowItem(pItem);

	if(pShadow)
	{
		pShadow->SetData(dwData,nSubItem);
	}
}

DWORD CTreeListCtrl::GetItemData2( CTreeListItem* pItem )
{
	// retrieve 32-bit data of item specified by pItem
	ASSERT( pItem != NULL );
	return pItem->GetData2();
}

void CTreeListCtrl::SetItemData2( CTreeListItem* pItem, DWORD dwData )
{
	// set 32-bit data of item specified by pItem
	ASSERT( pItem != NULL );
	pItem->SetData2( dwData );
}
...
void CTreeListCtrl::SetCheck( CTreeListItem* pItem, BOOL bCheck )
{
	...
	if (m_bCareCheck && m_pFuncTreeListCtrlItemChecked)
	{
		m_pFuncTreeListCtrlItemChecked(m_pOwner, this, pItem);
	}
}
...
BOOL CTreeListCtrl::GetItemRectTree( CTreeListItem* pItem, LPRECT lpRect )
{
	...
	// skip tree image
	if( m_dwStyle&TLC_IMAGE_PLUS )
		if( pItem->GetState()&TLIS_SHOWTREEIMAGE )
			nPerfix += TLL_WIDTH;


	// skip gaps between text and image/loc
	if( m_dwStyle&TLC_LOCKBOX || m_dwStyle&TLC_IMAGE || m_dwStyle&TLC_IMAGE_PLUS)
		nPerfix += 2;
...
DWORD CTreeListCtrl::GetItemData2( int nItem )
{
	// retrieve data of item ( list )
	ASSERT( 0 <= nItem && nItem < GetCount() );
	return GetItemData2( GetTreeListItem(nItem) );
}

void CTreeListCtrl::SetItemData2( int nItem, DWORD dwData )
{
	// set data of item ( list )
	ASSERT( 0 <= nItem && nItem < GetCount() );
	SetItemData2( GetTreeListItem(nItem), dwData );
}
...
void CTreeListCtrl::SetFont(CFont* pFont)
{
	if (!pFont)
	{
...
void CTreeListCtrl::SetItemBkColor(CTreeListItem * pItem, COLORREF clrBk)
{
	pItem->m_clrBk = clrBk;
	if(pItem->m_bShadow)
	{
...
void CTreeListCtrl::SetItemTextColor(CTreeListItem * pItem, COLORREF clr)
{
	pItem->m_clrTxt = clr;
	if(pItem->m_bShadow)
	{
...
void CTreeListCtrl::DrawItems( CDC* pDC, CRect rcClip )
{
	...
	if(m_nFixedCoumnCount>0)
	{
		DrawFixedCoumn(pDC,rcClip);
	}
	return;
}


void CTreeListCtrl::DrawFixedCoumn(CDC* pDC,CRect rcClip)
{
	CTreeListItem* pShowItem;
	...
	for( int iShow=0; iShow<m_nFixedCoumnCount; iShow++ )
	{
		CTreeListColumnInfo* pColumnInfo;
		int iCol;
	...
	rcItem.SetRect( 0, 0, nWidth, GetItemHeight() );
	rcItem.OffsetRect( m_rcTreeList.left, m_rcTreeList.top );
	//rcItem.OffsetRect( -GetScrollPos( SB_HORZ ), 0 );
	...
	for( int iRow=nFirstRow; iRow<= nFirstRow + nShowRows && iRow < GetVisibleCount(); iRow++, pShowItem = GetNextShowItem( pShowItem ) )
	{
		...
		DrawItem( pDC, rcItem, pShowItem,TRUE ); 
...
void CTreeListCtrl::DrawItem( CDC* pDC, CRect rcItem, CTreeListItem* pItem ,BOOL bDrawFixed)
{
	// draw item background
	rcItem.left = m_nPadTreeCol;
	...
	for( int iShow = 0; iShow < m_arShows.GetSize(); iShow++, nColPos += pColumnInfo->m_nWidth )
	{
		...
		if(iShow>=m_nFixedCoumnCount && bDrawFixed) continue;

		CRect rcColumn;
		rcColumn.SetRect( iCol == 0 ? m_nPadTreeCol : 0, rcItem.top, pColumnInfo->m_nWidth, rcItem.bottom );
		...
		else if(pItem->GetSelected())
		{
			...
			{
				pDC->FillSolidRect(&rcColumn,RGB(0,0,200));
			}
			else
			{
				pDC->FillSolidRect(&rcColumn,RGB(160,160,210));
			}
		}
		if( iCol == 0 )
		{
...
void CTreeListCtrl::DrawItemExclude( CDC* pDC, CRect rcItem, CTreeListItem* pItem )
{
	...
	for( int iShow = 0; iShow < m_arShows.GetSize(); iShow++, nColPos += pColumnInfo->m_nWidth )
	{
		...
		CRect rcColumn;
		rcColumn.SetRect( iCol == 0 ? rcItem.left : 0, rcItem.top, pColumnInfo->m_nWidth, rcItem.bottom );
...
void CTreeListCtrl::DrawItemTree( CDC* pDC, CRect rcColumn, CTreeListItem* pItem, int iCol )
{
	...
	//if (pItem->m_bShadow)
	//{
	//	ASSERT(pItem->m_pParent);
	//	rectCrossLine = CalcLineRect(rcColumn, pItem->m_pParent);
	//}
	if( m_dwStyle&(TLC_TREELINE|TLC_BUTTON) )
	{
		rcTree.left = rcColumn.left + nPerfix;
		...
		else
		{
			...
			if (pItem->m_bShadow)
			{
				//nShadowOffset = rectCrossLine.left-rcTree.left;
				rcTree += CPoint(nShadowOffset, 0);
				
				DWORD tmpC = m_crTreeLine;

				m_crTreeLine = RGB(0,0,165);

				// child item of other items ( not root item )
				if( pItem->m_pNext == NULL || !pItem->m_pNext->m_bShadow)
					DrawItemTreeLine( pDC, rcGraph, rcTree, TLL_RIGHT | TLL_TOP );					// ©»
				else
					DrawItemTreeLine( pDC, rcGraph, rcTree, TLL_RIGHT |TLL_TOP | TLL_BOTTOM );		// ©Ç
				...
				bool bHaveRealChild = false;
				CTreeListItem* pTemp = pItem->m_pNext;
				while (pTemp)
				{
					if (!pTemp->m_bShadow)
					{
		...
		if( m_dwStyle&TLC_BUTTON )
		{
			//if( pItem->GetState()&TLIS_EXPANDEDONCE )
			{
				if( pItem->m_pChild != NULL )
				{
						CTreeListItem* pCC = pItem->m_pChild;
	...
	if( m_dwStyle&TLC_CHECKBOX )
	{
		if( pItem->GetState()&TLIS_SHOWCHECKBOX )
		{
            ...
            if (pItem->m_bShadow)
            {
                rcCheckBox += CPoint(nShadowOffset, 0);
            }
			...
			int nCheck = pItem->GetCheck();
			CRect rcCheck(pt.x+2,pt.y+2,pt.x+15,pt.y+15);
			if(nCheck == 1)
			{
				pDC->Draw3dRect(&rcCheck,RGB(51,51,51),RGB(51,51,51));
				CPen ps(PS_SOLID,1,RGB(0,0,80));
				CPen ps2(PS_SOLID,1,RGB(0,0,110));
				CPen* pOldPen = pDC->SelectObject(&ps);

				pDC->MoveTo(rcCheck.left+2,rcCheck.top+5);
				pDC->LineTo(rcCheck.left+5,rcCheck.top+8);
				pDC->LineTo(rcCheck.left+11,rcCheck.top+2);
				
				pDC->SelectObject(&ps2);
				pDC->MoveTo(rcCheck.left+2,rcCheck.top+6);
				pDC->LineTo(rcCheck.left+5,rcCheck.top+9);
				pDC->LineTo(rcCheck.left+11,rcCheck.top+3);

				pDC->SelectObject(pOldPen);
			}
			else if(nCheck == 2)
			{
				pDC->FillSolidRect(&rcCheck,RGB(255,255,230));
				pDC->Draw3dRect(&rcCheck,RGB(51,51,51),RGB(51,51,51));
			
				
				CRect rcInner = rcCheck;
				rcInner.DeflateRect(3,3,3,3);
				
				pDC->FillSolidRect(&rcInner,RGB(185,183,0));
			}
			else
			{
				pDC->Draw3dRect(&rcCheck,RGB(51,51,51),RGB(51,51,51));
	...
	if( m_dwStyle&TLC_LOCKBOX )
	{
		if( pItem->GetState()&TLIS_SHOWLOCKBOX )
		{
            ...
            if (pItem->m_bShadow)
            {
                rcLockBox += CPoint(nShadowOffset, 0);
            }
	...
	if( m_dwStyle&TLC_IMAGE_PLUS )
	{
		if( pItem->GetState()&TLIS_SHOWTREEIMAGE )
		{
			...
			if (pItem->m_bShadow)
			{
				rcImage += CPoint(nShadowOffset, 0);
			}
			...
			{
				if( m_arSelects.GetSize() > 0 && pItem == (CTreeListItem*)m_arSelects[0] )
					m_pImageList->Draw( pDC, pItem->m_nExpandSelectedImagePlus, pt, nStyle );
				else
					m_pImageList->Draw( pDC, pItem->m_nSelectedImagePlus, pt, nStyle );
			}
			else
			{
				...
				if( m_arSelects.GetSize() > 0 && pItem == (CTreeListItem*)m_arSelects[0] )
					m_pImageList->Draw( pDC, pItem->m_nExpandImagePlus, pt, nStyle );
				else
					m_pImageList->Draw( pDC, pItem->m_nImagePlus, pt, nStyle );
			}
	...
	if( m_dwStyle&TLC_IMAGE )
	{
		if( pItem->GetState()&TLIS_SHOWTREEIMAGE )
		{
			...
			if(pItem->GetState() &TLIS_SELECTED )
			{
				//if(GetState( TLS_FOCUS ))
				//{
				//	DWORD crBkgnd;
				...
				//	pDC->FillSolidRect(pt.x,pt.y,rcImage.Width(),rcImage.Height(),crBkgnd);
				//}
				//else if(m_dwStyle&TLC_SHOWSELALWAYS)
				//{
				//	pDC->FillSolidRect(pt.x,pt.y,rcImage.Width(),rcImage.Height(),m_crBkSelectedNoFocus);
				//}
			}
			if( pItem->GetState()&TLIS_EXPANDED && pItem->m_pChild != NULL )
			{
	...
	if (pItem->m_shadowList.size() != 0)
	{
		CRect rcShadowBtn = rcGraph;
		rcShadowBtn.left = rcColumn.left + nPerfix;
		rcShadowBtn.right = rcShadowBtn.left + TLL_WIDTH;
		...
		if( m_dwStyle&TLC_BUTTON )
		{
			//if( pItem->GetState()&TLIS_EXPANDEDONCE )
			{
				//if( pItem->m_pChild != NULL )
				{
					if( pItem->m_bShowShadow &&  pItem->GetState()&TLIS_EXPANDED)
						DrawItemTreeButton( pDC, rcGraph, rcShadowBtn, TLB_MINUS );
					else
						DrawItemTreeButton( pDC, rcGraph, rcShadowBtn, TLB_PLUS );
				}				
			}
			//else
			//{
	...
	{
		pDC->MoveTo( rcColumn.left - (iCol  == 0 ?  m_nPadTreeCol : 0) , rcColumn.bottom - 1);
		pDC->LineTo( rcColumn.right-1, rcColumn.bottom - 1);
	}
...
void CTreeListCtrl::DrawItemText( CDC* pDC, CRect rcBkgnd, CRect rcText, CTreeListItem* pItem, int iCol )
{
	...
	if(iCol>0 && pItem->m_arColImage[iCol]>=0 )
	{
		UINT nStyle = ILD_TRANSPARENT;
		...
		if(rcText.Height()> 16)
		{
			pt.y += (rcText.Height() - 16)/2;
		}
		m_pImageList->Draw(pDC,pItem->m_arColImage[iCol],pt,nStyle);
		rcText.left += 18;
	}
...
CTreeListItem * CTreeListCtrl::GetItemRootNode(CTreeListItem * pItem)
{
	if (!pItem)
	{
		return NULL;
	}
	CTreeListItem * pRootItem = NULL;
	while (pItem->m_nLevel >= 1)
	{
		pRootItem = pItem;
		pItem = pItem->m_pParent;
	}
...
BOOL CTreeListCtrl::Expand( CTreeListItem* pItem, int nCode, int iSubItem )
{
	...
	case TLE_EXPAND:
		...
		StatExpand( pItem );
		if(pItem->m_bShowShadow)
		{
			ExpandItemShadow(pItem);
		}
		break;

	case TLE_COLLAPSE:
		CollapseItemShadow(pItem);
		pItem->Collapse();
...
CTreeListItem* CTreeListCtrl::InsertItemNew( LPCTSTR lpszItem, CTreeListItem* pParent )
{
	...
	if(pParent != m_pRootItem && pParent->GetCheck() == 1 )
		pNewItem->SetCheck( TRUE );

	if(pParent != m_pRootItem && pParent->GetLock() == 1 )
...
void CTreeListCtrl::SelectItem( CTreeListItem* pItem, int iSubItem, BOOL bClearLastSelected,BOOL bMoveScrll )
{
	LRESULT lResult;

	if (!m_bShiftMutiSelecet)
	{
		m_pShiftMutiSelecteBeginItem = NULL;
	}
		...
		if( pItem != NULL )
		{
			if( GetState( TLS_FOCUS ) && !(nFlags&(MK_LBUTTON | MK_RBUTTON | MK_MBUTTON)) )
			{
				...
				if (si.nMax-si.nMin>0)
				{
					...
					GetClientRect(&rcClient);
					int cy = ::GetSystemMetrics( SM_CYHSCROLL );
					if(rcText.bottom<rcClient.bottom)
					{
			...
			if( nFlag&TLHT_ONITEMBUTTON || nFlag&TLHT_ONSHADOWBUTTON)
			{
				SetCursor( m_hButton );
			} 
...
void CTreeListCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if( pItem != NULL )
	{
		if( m_dwStyle&TLC_TREELIST )
		{
			if (pItem->m_shadowList.size() != 0 && nFlag&TLHT_ONSHADOWBUTTON)
			{
				pItem->m_bShowShadow = !pItem->m_bShowShadow;
				if (pItem->m_bShowShadow)
				{
					ExpandItemShadow(pItem);
				}
		...
		{
			POSITION pos = GetFirstSelectedItemPosition();
			if(pos == NULL || GetSelectedCount()<2 || !pItem->GetSelected())
			{
				SetItemCheck( pItem );
			}
			else
			{
				bool bCheck = (pItem->GetCheck() != 0);
				while(pos)
				{
					CTreeListItem* pSel = GetNextSelectedItem(pos);
					if(pSel == NULL) break;
					bool bThis = (pSel->GetCheck() ==0);
					if(bThis != bCheck)
					{
						SetItemCheck(pSel);
					}
		...
		if( nFlag&TLHT_ONITEM && !(nFlag&TLHT_ONITEMBUTTON) && !(nFlag&TLHT_ONITEMCHECKBOX) && !(nFlag&TLHT_ONITEMLOCKBOX))
		{
			...
			if (nFlags&MK_SHIFT)
			{
				if (!m_pShiftMutiSelecteBeginItem)
				{
					m_pShiftMutiSelecteBeginItem = m_pFocusItem;
				}
				ShiftMuitiSelect(m_pShiftMutiSelecteBeginItem, pItem, iSubItem);
			}
			else
			{
				if( !pItem->GetSelected() )
				{
					...
					{
						SelectItem( pItem, iSubItem,TRUE,FALSE );
					}
					else
					{
						SelectItem( pItem, iSubItem, FALSE,FALSE );
					}
					bDraging = BeginDraging( point );
				}
				else
				{
					if((nFlags&MK_CONTROL) &&  (m_dwStyle&TLC_MULTIPLESELECT))
					{
						pItem->SetSelected( FALSE );
						for(int i=m_arSelects.GetCount()-1;i>=0;i--)
						{
							if(m_arSelects[i] == (void*)pItem)
							{
								m_arSelects.RemoveAt(i);
							}
						}
...
void CTreeListCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	else if(pItem)
	{
		if(bMyClick)
		{
			Notify(TLN_ITEM_CLICK, pItem, iSubItem);
		}
...
void CTreeListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if( pItem != NULL )
	{
		...
		LRESULT r= Notify(TLN_ITEM_DCLICK, pItem, iSubItem);
    ...
    if (m_bCareDblClk && m_pFuncTreeListCtrlDblClk)
    {
		m_pFuncTreeListCtrlDblClk(m_pOwner, this,  pItem);
    }
...
void CTreeListCtrl::OnRButtonUp(UINT nFlags, CPoint point) 
{
	...
	CTreeListItem * pItem = HitTest(point, &nFlag, &iSubItem, &rcText);
	Notify(TLN_ITEM_RCLICK, pItem, iSubItem);
...
void TreeListExpandItem(CTreeListCtrl* pCtrl, CTreeListItem *pItem)
{
	...
	while(pItem)
	{
		if(pItem->m_pChild)
		{
			pCtrl->Expand(pItem,TLE_EXPAND);
			TreeListExpandItem(pCtrl,pItem->m_pChild);
		}
...
{
	TreeListExpandItem(this,GetRootItem());
}
...
LRESULT CTreeListCtrl::OnNcHitTest(CPoint point)
{
  /*  if (m_resizeMoveFlag)
    {
	    #include "..\WndResize.h"
...
DROPEFFECT CTreeListCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{

    OutputDebugString(_T("___ OnDragEnter"));

    DROPEFFECT dwEffect = pDataObject->GetGlobalData(CF_HDROP) ? DROPEFFECT_COPY : DROPEFFECT_NONE;
...
DROPEFFECT CTreeListCtrl::OnDragOver(COleDataObject* pDataObject,DWORD dwKeyState, CPoint point)
{
	OutputDebugString(_T("___ OnDragOver"));

    CPoint pointScr = point;
    ClientToScreen(&pointScr);
    HighlightOverItem(pointScr);

	HGLOBAL hglobal = pDataObject->GetGlobalData(CF_HDROP);

    DROPEFFECT dwEffect = pDataObject->GetGlobalData(CF_HDROP) ? DROPEFFECT_COPY : DROPEFFECT_NONE;

	if (m_pDragItems && m_pDragItems->size()>0)
	{
...
void CTreeListCtrl::OnDragLeave()
{
	OutputDebugString(_T("___ OnDragLeave"));
...
BOOL CTreeListCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
	OutputDebugString(_T("___ OnDrop"));

	vector<CString> items;
	BOOL bRet = ReadHdropData(pDataObject, items);
	...
	InsertDropItems(items);

    return bRet;
}

DROPEFFECT CTreeListCtrl::OnDropEx(COleDataObject* pDataObject, DROPEFFECT dropDefault, DROPEFFECT dropList, CPoint point)
{
    OnDrop(pDataObject, dropDefault,  point);
	return DROPEFFECT_NONE;
}

DROPEFFECT CTreeListCtrl::OnDragScroll(DWORD dwKeyState, CPoint point)
{
	return DROPEFFECT_NONE;
}

BOOL CTreeListCtrl::ReadHdropData(COleDataObject* pDataObject, vector<CString> &files)
{
	HGLOBAL     hg;
	HDROP       hdrop;
	...
	for (UINT uFile = 0; uFile < DragQueryFile(hdrop, -1, NULL, 0); uFile++)
    {
...
void CTreeListCtrl::HighlightOverItem(CPoint point)
{
    ScreenToClient(&point);
    CTreeListItem* item = NULL;
    if ((item = HitTest(point)) != NULL)
    {
        SelectItem(item, 0, true);
    }
}

CShadowItem* CTreeListCtrl::AddShadowItem( LPCTSTR lpszItem, CTreeListItem* pParent,int nImage )
{
	if (pParent)
	{
		CShadowItem* pItem = new CShadowItem(pParent);
		
		pItem->SetImage(nImage);
		pItem->m_arrTxt.resize(m_arColumns.GetCount());
		pItem->m_arrData.resize(m_arColumns.GetCount(),0);
		pItem->SetText(lpszItem);
		pParent->m_shadowList.push_back(pItem);
...
CShadowItem* CTreeListCtrl::FindShadowItem(CTreeListItem* pItem)
{
	if(!pItem->m_bShadow) return NULL;
	if(pItem->m_pParent==NULL) return NULL;

	for(int i=0;i<pItem->m_pParent->m_shadowList.size();i++)
	{
		if(pItem->m_pParent->m_shadowList[i]->m_pRealNode == pItem)
		{
			return pItem->m_pParent->m_shadowList[i];
		}
	}
...
int CTreeListCtrl::GetShadowCount(CTreeListItem* pItem)
{
	return pItem->m_shadowList.size();
}

void CTreeListCtrl::ShiftMuitiSelect(CTreeListItem* pItem1, CTreeListItem* pItem2, int iSubItem)
{
	ASSERT(pItem1 && pItem2);

	m_bShiftMutiSelecet = true;
	CTreeListItem* pShowItem;
	...
	if (pItem1 == pItem2)
	{
		SelectItem(pItem1, iSubItem, FALSE);
		return;
	}

	pShowItem = GetFirstShowItem( 0 );

	bool bSelect = false;
	for( int iRow=0; iRow < GetVisibleCount(); iRow++, pShowItem = GetNextShowItem( pShowItem ) )
	{
		ASSERT( pShowItem != NULL );

		bool bBeginOrEnd = pShowItem == pItem1 || pShowItem == pItem2;

		// end item
		if (bBeginOrEnd && bSelect)
		{
			if (!pShowItem->GetSelected())
			{
				SelectItem(pShowItem, iSubItem, FALSE);
			}
			break;
		}
...
void CTreeListCtrl::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	m_nHitTest = nHitTest;
...
void CTreeListCtrl::InsertDropItems(vector<CString> &files)
{
	if(m_pFileDropHandler)
	{
		CTreeListItem* pSelItem = GetSelectedItem();
		m_pFileDropHandler->OnFileDrop(this,pSelItem,files);
	...
	else
	{
		...
		if (pSelItem)
		{
			CTreeListItem* pAfter = pSelItem;
			for (vector<CString>::iterator it = files.begin(); it != files.end(); ++it)
			{
				pAfter = InsertItem(*it, pSelItem, pAfter);
				SetItemImage(pAfter, 4, 5, 6, 7);
				SetItemState(pAfter, 0, TLIS_SHOWCHECKBOX | TLIS_SHOWLOCKBOX);

			}
			Expand(pSelItem, TLE_EXPAND);
		}
	}
...
void CTreeListCtrl::ExpandItemShadow(CTreeListItem* pItem)
{
	CollapseItemShadow(pItem);
	CTreeListItem* pAfter = pItem;
	for (int i=0; i<pItem->m_shadowList.size();i++)
	{
		CShadowItem* pShadow = pItem->m_shadowList[i];

		pAfter = InsertItem(pShadow->GetText(), pItem, pAfter);
		pAfter->m_bShadow = true;
		pAfter->m_strData = pShadow->m_strData;
		
		pShadow->m_pRealNode = pAfter;
		
		for(int j=0;j<pShadow->m_arrTxt.size();j++)
		{
			pAfter->SetText(pShadow->m_arrTxt[j],j);
			pAfter->SetData(pShadow->m_arrData[j],j);
		}
		int nImg = pShadow->m_nImage;
		SetItemImage(pAfter, nImg,nImg,nImg,nImg);
		
		SetItemState(pAfter, 0, TLIS_SHOWCHECKBOX | TLIS_SHOWLOCKBOX);
		if(nImg<0)
		{
			pAfter->SetState(NULL,TLIS_SHOWTREEIMAGE );
		}

		SetItemTextColor(pAfter,pShadow->m_clrTxt);
		SetItemBkColor(pAfter,pShadow->m_clrBk);
	}
	pItem->m_bShowShadow = true;
	Expand(pItem, TLE_EXPAND);
	
}

void CTreeListCtrl::CollapseItemShadow(CTreeListItem* pItem)
{
	CPtrArray arItem;
	CTreeListItem* pShadow = pItem->m_pChild;
	
	while (pShadow)
	{
		if (pShadow->m_bShadow)
		{
			arItem.Add(pShadow);
		}
		pShadow = pShadow->m_pNext;
	}
	
	for(int i=0;i<pItem->m_shadowList.size();i++)
	{
		pItem->m_shadowList[i]->m_pRealNode = NULL;
	}
	for (int j = 0; j < arItem.GetSize(); ++j)
	{
		DeleteItem((CTreeListItem*)arItem[j]);
	}

	pItem->m_bShowShadow = false;
}

void CTreeListCtrl::MoveUp( CTreeListItem* pItem )
{
	if (pItem)
	{
		CTreeListItem* pPrev = pItem->m_pPrev;

		if (pPrev)
		{
			MoveAfterItem( pPrev, pItem );
		}
	}
}

void CTreeListCtrl::MoveDown( CTreeListItem* pItem )
{
	if (pItem)
	{
		CTreeListItem* pNext = pItem->m_pNext;

		if (pNext)
		{
			MoveAfterItem( pItem, pNext );
		}
	}
}

void CTreeListCtrl::MoveTop( CTreeListItem* pItem )
{
	if (pItem)
	{
		MoveAfterItem( pItem, NULL );
	}
}

void CTreeListCtrl::OnWndActive()
{

}


CTreeListItem* CTreeListCtrl::NextItem( CTreeListItem* pItem, bool bChild)
{
	ASSERT( pItem != NULL );
	...
	if( pItem != m_pRootItem )
	{
		pItem = pItem->m_pParent;
		if( pItem != m_pRootItem )
			return NextItem( pItem, false );
	}

	return NULL;
}

void CTreeListCtrl::CreateContextMenu(CPoint point)
{
	int nFlag, iSubItem;
	CRect rcText;
	m_pItemContextMenuOn = HitTest( point, &nFlag, &iSubItem, &rcText );

	ClientToScreen(&point);
	m_menu.DestroyMenu();
	m_menu.CreatePopupMenu();

	if (m_pFuncTreeListCtrlCreateContextMenu)
	{
		m_pFuncTreeListCtrlCreateContextMenu(m_pOwner, this, m_pItemContextMenuOn, &m_menu);
	}
...
BOOL CTreeListCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	int nCode = HIWORD(wParam);;
	UINT nID = LOWORD(wParam);
	if (CN_COMMAND == nCode)
	{
		if (m_bContextMenu && m_pFuncTreeListCtrlContextMenuClick)
		{
			m_pFuncTreeListCtrlContextMenuClick(m_pOwner, this, m_pItemContextMenuOn, nID);
		}
	}
...
void CTreeListCtrl::CloseOleDropTarget(BOOL bClose/*=TRUE*/)
{
    m_bCloseOleDropTarget = bClose;
}


void CTreeListCtrl::SetPadTreeColumn(int nPadCol)
{
	m_nPadTreeCol = nPadCol;
}

void CTreeListCtrl::SetFixedCoumnCount(int nCount)
{
	m_nFixedCoumnCount = nCount;
}