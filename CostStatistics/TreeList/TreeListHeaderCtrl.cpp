...
BOOL CTreeListHeaderCtrl::DrawHeaders( CDC* pDC )
{
	...
	int nFixedCount = m_pTreeListCtrl->m_nFixedCoumnCount;
	if(nFixedCount>0)
	{
		int nHeaderPos = 0;
		for( int iShow=0; iShow<nFixedCount; iShow++, nHeaderPos += pColumnInfo->m_nWidth )
		{
			int iCol;
...
BOOL CTreeListHeaderCtrl::DrawHeader( CDC* pDC, CRect rcHeader, int iCol )
{
	...
	if(m_pTreeListCtrl->GetEnableFilter())
	{
		CRect rcFilter = rcHeader;
		rcFilter.left = rcFilter.right - rcHeader.Height();
		rcFilter.DeflateRect(4,4,4,4);
		rcFilter.MoveToX(rcFilter.left+6);

		pDC->Draw3dRect(&rcFilter,RGB(130,130,160),RGB(130,130,160));
		
		CPoint ct = rcFilter.CenterPoint();
		CBrush br(RGB(0,0,120));
		CPen pen(PS_SOLID,1,RGB(0,0,120));

		CBrush *pOldBr = pDC->SelectObject(&br);
		CPen *pOldPen = pDC->SelectObject(&pen);
		
		
		if(m_arrFilter.count(iCol)>0)
		{
			POINT pts[] = {{ct.x-5,ct.y-5},{ct.x+5,ct.y-5},{ct.x+1,ct.y-1},{ct.x+1,ct.y+6},{ct.x-1,ct.y+4},{ct.x-1,ct.y-1}};
			pDC->Polygon(pts,6);
		}
		else
		{
			POINT pts[] = {{ct.x-3,ct.y-2},{ct.x+3,ct.y-2},{ct.x,ct.y+3}};
		...
		rcHeader.right=rcFilter.left-4;
	}
	// draw sort arrow
...
LRESULT CTreeListHeaderCtrl::OnLayout(WPARAM wParam, LPARAM lParam)
{
	...
	if(rcOld.left == pRect->left && rcOld.top==pRect->top && rcOld.Width() == pRect->Width() && rcOld.Height() == pRect->Height())
	{
		return TRUE;
	}
	::MoveWindow(
...
void CTreeListHeaderCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if( !m_bSizing && !m_bDraging )
	{
		...
		{
			if(iFlag & TLHT_ONHEADFILTER)
			{
				int ret = m_pTreeListCtrl->Notify(TLN_HEAD_FILTER, NULL, iCol);

				if(ret ==0)
				{
					m_arrFilter.erase(iCol);
				}
				else
				{
					m_arrFilter.insert(iCol);
				}
				Invalidate();
				return;
			}
			else if( iFlag&TLHT_ONHEADER )
			{
				m_iCol = iCol;
				BeginDraging( point );
			}
...
void CTreeListHeaderCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if( !m_bSizing && !m_bDraging )
	{
		...
		else
		{
			...
			bool bRet =	HitTest( point,&iFlag,&iCol);
			...
			if(bRet)
			{
				m_pTreeListCtrl->Notify(TLN_HEAD_DCLICK, NULL, iCol);
			}
		}
	}
}
...
BOOL CTreeListHeaderCtrl::HitTestSize( CPoint pt, int* pFlag, int* pCol )
{
	...
	*pCol	= 0;
	...
	for( int iShow = 0; iShow<arShows.GetSize(); iShow++ )
	{
		if(iShow<m_pTreeListCtrl->m_nFixedCoumnCount)
		{
			nPos = pt.x;
		}
		else
...
BOOL CTreeListHeaderCtrl::HitTestGaps( CPoint pt, int* pFlag, int* pCol )
{
	...
	*pCol	= 0;
	...
	{
		if(0<m_pTreeListCtrl->m_nFixedCoumnCount)
		{
			nPos = pt.x;
		}
		else
	...
	*pCol = 0;
	...
	for( int iShow = 0; iShow<arShows.GetSize(); iShow++ )
	{
		...
		if( rcColumn.PtInRect( pt ) )
		{
			...
			{
				CRect rcFilter = rcHeader;
				rcFilter.left = rcFilter.right-rcFilter.Height();
				rcFilter.DeflateRect(4,4,4,4);
				rcFilter.MoveToX(rcFilter.left+6);
				if(rcFilter.PtInRect(pt))
				{
					*pFlag = TLHT_ONHEADFILTER;
					*pCol = iCol;
...
void CTreeListHeaderCtrl::SetBkColor(COLORREF clr)
{
    m_cr3DFace = clr;
    Invalidate();
}
