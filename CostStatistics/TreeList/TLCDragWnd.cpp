...
BOOL CTLCDragWnd::Create( CRect rcWindow, CTreeListCtrl* pTreeListCtrl )
{
	...
	else
	{
		...
		int wid = rcClient.Width();
		if(m_pTreeListCtrl->GetColumnCount()>0)
		{
			wid = m_pTreeListCtrl->GetColumnWidth(0);
			wid = max(60,wid);
		}
		if( !CreateEx( dwExStyle, TLCDRAGWND_CLASSNAME, NULL, dwStyle,
			rcWindow.left, rcWindow.top, wid, pTreeListCtrl->GetItemHeight(), NULL, NULL, NULL ) )
			return FALSE;
	}
