...
BEGIN_MESSAGE_MAP(zt_CScrollView, zt_CView)
	...
	ON_WM_MOUSEWHEEL()
	//}}AFX_MSG_MAP
	ON_MESSAGE(WM_MBUTTONDOWN, &zt_CScrollView::HandleMButtonDown)
END_MESSAGE_MAP()
...
/*extern*/ BOOL _zt_afxGotScrollLines = FALSE; // defined in wincore.cpp

UINT PASCAL _zt_AfxGetMouseScrollLines()
{
...
class _zt_AFX_MOUSEANCHORWND : public CWnd
{
private:
   using CWnd::Create;

public:
	_zt_AFX_MOUSEANCHORWND(CPoint& ptAnchor);
	~_zt_AFX_MOUSEANCHORWND();

	BOOL Create(zt_CScrollView* pParent);
...
BEGIN_MESSAGE_MAP(_zt_AFX_MOUSEANCHORWND, CWnd)
	ON_WM_PAINT()
	ON_WM_TIMER()
END_MESSAGE_MAP()

_zt_AFX_MOUSEANCHORWND::_zt_AFX_MOUSEANCHORWND(CPoint& ptAnchor)
: m_ptAnchor(ptAnchor), m_bQuitTracking(FALSE)
{
}

_zt_AFX_MOUSEANCHORWND::~_zt_AFX_MOUSEANCHORWND()
{
}

BOOL _zt_AFX_MOUSEANCHORWND::PreTranslateMessage(MSG* pMsg)
{
...
void _zt_AFX_MOUSEANCHORWND::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (m_bQuitTracking)
	{
		...
		ReleaseCapture();
		SetCursor(NULL);
		zt_CScrollView* pView = (zt_CScrollView*) GetOwner();
		DestroyWindow();
	...
	else
	{
		...
		else
		{
			...
			pSplitter->DoScrollBy((CView*)pView, sizeToScroll, TRUE); ///!!!
		}	
...
void _zt_AFX_MOUSEANCHORWND::SetBitmap(UINT nID)
{
...
BOOL _zt_AFX_MOUSEANCHORWND::Create(zt_CScrollView* pParent)
{
	ENSURE_VALID(pParent);
	ASSERT_KINDOF(zt_CScrollView, pParent);
...
void _zt_AFX_MOUSEANCHORWND::OnPaint()
{
	CPaintDC dc(this);
	CRect rect;
	GetClientRect(&rect);
...
zt_CScrollView::zt_CScrollView()
{
	// Init everything to zero
	AFX_ZERO_INIT_OBJECT(zt_CView);
...
void zt_CScrollView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo)
{
	ASSERT_VALID(pDC);
	...
	zt_CView::OnPrepareDC(pDC, pInfo);     // For default Printing behavior
}

/////////////////////////////////////////////////////////////////////////////
// Set mode and scaling/scrolling sizes

void zt_CScrollView::SetScaleToFitSize(SIZE sizeTotal)
{
...
const AFX_DATADEF SIZE zt_CScrollView::sizeDefault = {0,0};

void zt_CScrollView::SetScrollSizes(int nMapMode, SIZE sizeTotal,
...
CPoint zt_CScrollView::GetScrollPosition() const   // logical coordinates
{
...
void zt_CScrollView::ScrollToPosition(POINT pt)    // logical coordinates
{
...
CPoint zt_CScrollView::GetDeviceScrollPosition() const
{
...
void zt_CScrollView::GetDeviceScrollSizes(int& nMapMode, SIZE& sizeTotal,
...
void zt_CScrollView::ScrollToDevicePosition(POINT ptDev)
{
	ASSERT(ptDev.x >= 0);
	ASSERT(ptDev.y >= 0);
...
CSize zt_CScrollView::GetWheelScrollDistance(CSize sizeDistance, BOOL bHorz, BOOL bVert)
{
	CSize sizeRet;
...
void zt_CScrollView::FillOutsideRect(CDC* pDC, CBrush* pBrush)
{
	ASSERT_VALID(pDC);
	ASSERT_VALID(pBrush);
...
void zt_CScrollView::ResizeParentToFit(BOOL bShrinkOnly)
{
...
void zt_CScrollView::OnSize(UINT nType, int cx, int cy)
{
	zt_CView::OnSize(nType, cx, cy);
	if (m_nMapMode == MM_SCALETOFIT)
	{
...
LRESULT zt_CScrollView::HandleMButtonDown(WPARAM wParam, LPARAM lParam)
{
	...
	{
		zt_CView::OnMButtonDown(nFlags, point);
		return FALSE;
	}
	...
	else
	{
		if (m_pAnchorWindow == NULL)
		{
			...
			{
				m_pAnchorWindow = new _zt_AFX_MOUSEANCHORWND(point);
...
void zt_CScrollView::CenterOnPoint(CPoint ptCenter) // center in device coords
{
	CRect rect;
...
void zt_CScrollView::GetScrollBarSizes(CSize& sizeSb)
{
	...
	if (GetScrollBarCtrl(SB_VERT) == NULL)
	{
		...
		sizeSb.cx = zt_afxData.cxVScroll;
	...
	if (GetScrollBarCtrl(SB_HORZ) == NULL)
	{
		...
		sizeSb.cy = zt_afxData.cyHScroll;
		if (dwStyle & WS_BORDER)
			sizeSb.cy -= AFX_CY_BORDER;
	}
}

BOOL zt_CScrollView::GetTrueClientSize(CSize& size, CSize& sizeSb)
...
void zt_CScrollView::GetScrollBarState(CSize sizeClient, CSize& needSb,
...
void zt_CScrollView::UpdateBars()
{
...
void zt_CScrollView::CalcWindowRect(LPRECT lpClientRect, UINT nAdjustType)
{
...
void zt_CScrollView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void zt_CScrollView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar != NULL && pScrollBar->SendChildNotifyLastMsg())
		return;     // eat it
...
BOOL zt_CScrollView::OnMouseWheel(UINT fFlags, short zDelta, CPoint point)
{
...
void zt_CScrollView::CheckScrollBars(BOOL& bHasHorzBar, BOOL& bHasVertBar) const
{
	DWORD dwStyle = GetStyle();
...
BOOL zt_CScrollView::DoMouseWheel(UINT fFlags, short zDelta, CPoint point)
{
	UNUSED_ALWAYS(point);
	...
	BOOL bResult = FALSE;
	UINT uWheelScrollLines = _zt_AfxGetMouseScrollLines();
...
BOOL zt_CScrollView::OnScroll(UINT nScrollCode, UINT nPos, BOOL bDoScroll)
{
	// calc new x position
...
BOOL zt_CScrollView::OnScrollBy(CSize sizeScroll, BOOL bDoScroll)
{
	int xOrig, x;
	int yOrig, y;
...
#ifdef _DEBUG
void zt_CScrollView::Dump(CDumpContext& dc) const
{
	zt_CView::Dump(dc);
...
void zt_CScrollView::AssertValid() const
{
	zt_CView::AssertValid();
...
IMPLEMENT_DYNAMIC(zt_CScrollView, zt_CView)
